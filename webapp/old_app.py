...
def home():
    if request.args == {}:
      ...
      if filters.get("SchoolRadio") == None:
        query = "select zipcode, %s, avg(school_ratings), score from entries where school_ratings >= %d and %s between %d and %d group by 1,2,4" % (house_type, rating_min, house_type, min_price, max_price)
        ...
        query = "select zipcode, %s, avg(school_ratings), score from entries where grade_level like %s and school_ratings >= %d and %s between %d and %d group by 1,2,4" % (house_type, school_level, rating_min, house_type, min_price, max_price)
        rows = g.db.execute(query).fetchall()
        return render_template('webapp.html', filters=filters, rows=rows)
